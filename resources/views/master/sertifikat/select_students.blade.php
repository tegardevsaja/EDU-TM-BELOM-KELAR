<x-layouts.app :title="'Pilih Data Siswa'">
    <div class="max-w-6xl mx-auto p-6">
        ...
        <div class="mb-4 text-sm text-gray-600 dark:text-gray-300">
            <p>Langkah: Pilih Template Sertifikat → <strong>Pilih Siswa (opsional)</strong> → Pilih Template Nilai <strong>(opsional)</strong> → Customize</p>
            <p class="mt-1">Anda dapat melanjutkan tanpa memilih siswa. Sertifikat tetap bisa dicetak (tanpa data siswa tertentu).</p>
        </div>

        <form method="GET" action="{{ route('master.sertifikat.select_grade', $template->id) }}" id="studentsForm" class="space-y-5">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div class="md:col-span-1">
                    <label class="block text-sm font-medium mb-1">Filter Kelas <span class="text-gray-400 text-xs">(Opsional)</span></label>
                ...
                <div class="md:col-span-3">
                    <label class="block text-sm font-medium mb-2">Pilih Siswa <span class="text-gray-400 text-xs">(Opsional)</span></label>
                    <div class="mb-3 flex items-center gap-2">
                        <input type="text" id="studentSearch" placeholder="Cari nama/NIS..." class="w-full md:w-1/2 rounded border px-3 py-2" oninput="filterByName(this.value)">
                        <button type="button" class="rounded border px-3 py-2 text-sm" onclick="document.getElementById('studentSearch').value=''; filterByName('')">Clear</button>
                    </div>
            ...
            <div class="flex justify-end gap-2">
                ...
                <button type="submit" class="rounded bg-blue-600 px-4 py-2 text-white">Lanjut (Template Nilai Opsional)</button>
            </div>
        </form>
    </div>
    ...
    <script>
        ...
        function filterByKelas(kelasId) {
            ...
            document.querySelectorAll('[data-kelas]').forEach(el => {
                ...
                const matchesKelas = !currentKelas || kid === currentKelas;
                el.style.display = (matchesSearch && matchesKelas) ? '' : 'none';
            });
        }
